...
1. Explain what a token is used for.
  A token is used for web authentication to allow someone to login to a protected website that requires an account to access it.

2. What steps can you take in your web apps to keep your data secure?
  the first thing that is important for more than just security butt keeping your code up to date is crucial. Keeping your code clean also is good for security and for your site overall. Using a secure host/API is also very helpful to keep a website secure.

3. Describe how web servers work.
  They work by sending a web app files that are hosted on that server when requested using an endpoint within the code of the server. 

4. Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
  Create - goes along with PUT and POST requests.
  Read - goes with GET requests
  Update - uses PUT as well but specifically to update the UI that is already rendered
  Delete - unsurprisingly goes with the DELETE requests
